      ...
      case Some(profiles) =>
        ...
        val testDownloadJob = (v2rayProfiles: List[Profile]) => {
          val links = v2rayProfiles.map {
            ...
            case p if p.isVmess => VmessQRCode(p.v_v, p.name, p.v_add, p.v_port, p.v_id, p.v_aid, p.v_net, p.v_type, p.v_host, p.v_path, p.v_tls, p.v_security,null,"", p.t_allowInsecure).toString
